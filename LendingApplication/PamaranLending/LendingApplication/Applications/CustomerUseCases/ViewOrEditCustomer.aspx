<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ViewOrEditCustomer.aspx.cs" Inherits="LendingApplication.Applications.CustomerUseCases.ViewOrEditCustomer" %>
...
<head id="PageHeader" runat="server">
    ...
    <script type="text/javascript">
        ...
        Ext.onReady(function () {
            //enableDisablePanelElement(EmploymentInformationPanel, false);
            //enableDisablePanelElement(BasicInformationPanel, false);
            window.proxy = new ntfx.MIFMessagingProxy();
            window.proxy.init(['onpickcustomer', 'onpickemployer', 'addcustomerclassification']);
            window.proxy.on('messagereceived', onMessageReceived);


            maskUnmask(BasicInformationPanel1, false);
            maskUnmask(OtherSourceOfIncomePanel, false);
            maskUnmask(IdsPanel, false);
            maskUnmask(ContactInformationPanel, false);
            maskUnmask(EmploymentInformationPanel, false);
            maskUnmask(SpouseInformationPanel, false);
            maskUnmask(RemarksPanel, false);
        ...
        var onFormValidated = function (valid) {
                ...
                if (valid && btnOpen.getText() != 'Edit') {
                    ...
                    PageFormPanelStatusBar.setStatus({ text: 'Please fill out the forms.' });
                    btnSave.disable();
                }
        }
        ...
        var onMessageReceived = function (msg) {
            if (msg.tag == 'onpickemployer') {
                ...
                X.FillEmploymentDetails(msg.data.EmployerID);
        ...
        var saveSuccessful = function () {
            showAlert('Status', 'Customer record was successfully modified.', function () {
        ...
        var checkFailure = function () {
            showConfirm('Confirm', 'A party record with the same name already exists in the pick list. Do you want to create another party record with the same name?',
                function (response) {
                    if (response == 'yes') {
        ...
        var enableDisablePanel = function (panel, enable) {
            if (enable == true) {
                panel.enable();
            } else {
                panel.disable();
            ...
            if (enable == true) {
                panel.cascade(function (item) {
                    ...
                    if (index == -1) {
                        ...
                        } else if (item.getXType() == 'compositefield') {
                            item.items.each(function (child) {
                                var ind = compState.indexOf(child);
                                if (ind == -1) {
                                    if (child.getXType() == 'button'
                                    || child.getXType() == 'checkbox'
                                    || child.getXType() == 'radiogroup'
                                    || child.getXType() == 'radio') {
                                        child.enable();
                                    }
                                    else if (child.getXType() == 'datefield'
                                    || child.getXType() == 'combo')
                                        child.setReadOnly(false);
                                    else if (child.isFormField && (typeof child.readOnly != 'undefined')) {
                                        child.setReadOnly(false);
                                    }
                                }
                            });
        ...
        var searchPreviousState = function (panel) {
            panel.cascade(function (item) {
                ...
                } else if (item.getXType() == 'datefield' || item.getXType() == 'combo' || (item.isFormField && item.getXType() != 'compositefield')) {
                    if (item.readOnly) {
                        alert(item.getXType());
                        actualState.push(item);
                        if (item.hasListener('focus')) {
                            alert(item.getXType() + ' has listener');
                            compHasListeners.push(item);
                        }
                    }
                } else if (item.getXType() == 'compositefield') {
                    alert(item.getXType());
                    item.items.each(function (child) {
                        alert(child.getXType());
                        if (child.getXType() == 'datefield' || child.getXType() == 'combo' || (child.isFormField && child.getXType() != 'compositefield')) {
                            if (child.readOnly) {
                                alert(child.getXType() + ' add readonly');
                                compState.push(child);
                                if (child.hasListener('focus')) {
                                    alert(child.getXType() + ' has listener');
                                    compHasListeners.push(child);
                                }
                            }
                        }
                    });
        ...
        var openOrEdit = function () {
            ...
            maskUnmask(BasicInformationPanel1, enable);
            maskUnmask(OtherSourceOfIncomePanel, enable);
            maskUnmask(IdsPanel, enable);
            maskUnmask(ContactInformationPanel, enable);
            maskUnmask(EmploymentInformationPanel, enable);
            maskUnmask(SpouseInformationPanel, enable);
            maskUnmask(RemarksPanel, enable);

            if (enable) {
                btnOpen.setText('Open');
                flupCustomerImage.show();
                btnUpload.show();
            }
            else {
                btnOpen.setText('Edit');
                flupCustomerImage.hide()
                btnUpload.hide()
                var idss = RecordID.getValue();
                X.FillDetails(idss);
            }
        };
...
<body>
    ...
    <ext:Hidden ID="hiddenMode" runat="server" />
    ...
    <ext:Viewport ID="PageViewPort" runat="server" Layout="Fit" AutoHeight="true">
        <Items>
            <ext:FormPanel ID="RootPanel" runat="server" Layout="FitLayout"
                Border="false" MonitorValid="true" MonitorPoll="500" AutoScroll="true">
                <TopBar>
                ...
                <LoadMask Msg="Retrieving customer information..." ShowMask="true" />
                <Items>
                    <ext:TabPanel ID="PageTabPanel" DeferredRender="false" runat="server" Padding="0" HideBorders="true"
                        ...
                        <Items>
                            ...
                            <ext:Panel ID="BasicInformationPanel"  LabelWidth="200" runat="server"
                                Layout="Column" RowHeight=".5" Padding="5" Enabled="false" AutoScroll="true">
                                <Items>
                                    <ext:Panel runat="server" Border="false" Header="false"
                                                ColumnWidth=".25" Padding="5" AutoScroll="true">
                                        <Items>
                                            <ext:Image ID="PersonImageFile" Height="200" Width="230"
                                                runat="server" />
                                            <ext:FileUploadField ID="flupCustomerImage" Width="230" Height="35" Hidden="true" Icon="Attach" runat="server">
                                            </ext:FileUploadField>
                                            <ext:Button ID="btnUpload" Text="Upload" Hidden="true" Icon="ImageAdd" runat="server" Width="60">
                                                <DirectEvents>
                                        ...
                                        <ext:Panel ID="Panel1" runat="server" LabelWidth="100" Layout="FormLayout" Border="false" ColumnWidth=".6" AutoScroll="true">
                                    <Items>
                                            <ext:TextField ID="txtPersonName"  ReadOnly="true" FieldLabel="Name" Height="20" AllowBlank="false"
                                    ...
                                    <ext:CompositeField ID="cmfDistrict"  runat="server" Height="25" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtDistrict" AllowBlank="false" FieldLabel="District" Height="22" runat="server" ReadOnly="true" Flex="1">
                                                <Listeners>
                                                    <Focus Handler="#{wndDistrictList}.show();" />
                                                </Listeners>
                                            </ext:TextField>
                                            <ext:Button ID="btnBrowseDistrict" Text="Browse.." Height="22" runat="server" Width="60">
                                                <Listeners>
                                    ...
                                    <ext:RadioGroup ID="rdGender" runat="server" Width="300" FieldLabel="Gender" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:Radio ID="rdFemale"  runat="server" BoxLabel="Female" Checked="true" />
                                            <ext:Radio ID="rdMale" runat="server" BoxLabel="Male" />
                                        </Items>
                                    </ext:RadioGroup>
                                    <ext:DateField ID="datBirthdate" AllowBlank="false" runat="server"
                                        FieldLabel="Birthdate" AnchorHorizontal="60%" MsgTarget="Side"
                                        Editable="false">
                                            <DirectEvents>
                                                <Change OnEvent="checkAge" />
                                            </DirectEvents>
                                    </ext:DateField>
                                    <ext:CompositeField ID="cmfBirthplace"  runat="server" Height="25" FieldLabel="BirthPlace" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtBirthPlace" AllowBlank="true" Height="22" FieldLabel="Birthplace" runat="server"
                                                Flex="1" ReadOnly="true">
                                                <Listeners>
                                                    <Focus Handler="#{winAddressDetailB1}.show();" />
                                                </Listeners>    
                                            </ext:TextField>
                                            <ext:Button ID="Button2" Text="..." runat="server" Height="22" Width="30">
                                                <Listeners>
                                    ...
                                    <ext:ComboBox ID="cmbNationality" EmptyText="" AllowBlank="true" FieldLabel="Nationality"
                                        ...
                                        ForceSelection="false" Editable="false" TypeAhead="true" Mode="Local" TriggerAction="All"
                                        SelectOnFocus="true" />
                                    <ext:CompositeField ID="cmfTin"  runat="server" FieldLabel="TIN" Height="20" AnchorHorizontal="59.95%">
                                        <Items>
                                            <ext:NumberField ID="txtTin1" MaxLength="3" TabIndex="1" Height="20" Text="" runat="server" Flex="1" />
                                            <ext:DisplayField ID="DisplayField1" Height="20" runat="server" Text="-" Cls="dot-label" />
                                            <ext:NumberField ID="txtTin2" MaxLength="3" TabIndex="2" Height="20" Text="" runat="server" Flex="1" />
                                            <ext:DisplayField ID="DisplayField2" runat="server" Height="20" Text="-" Cls="dot-label" />
                                            <ext:NumberField ID="txtTin3" MaxLength="3" TabIndex="3" Text="" Height="20" runat="server" Flex="1" />
                                            <ext:DisplayField ID="DisplayField3" runat="server" Height="20" Text="-" Cls="dot-label" />
                                            <ext:NumberField ID="txtTin4" MaxLength="4" TabIndex="4" Text="" Height="20" runat="server" Flex="1" />
                                        </Items>
                                    </ext:CompositeField>
                                    ...
                                    <ext:TextField ID="txtCreditLimit" Text="" AllowBlank="true" FieldLabel="Credit Limit"
                                        runat="server" MaskRe="/[0-9\-\,\.]/" AnchorHorizontal="60%">
                                        <Listeners>
                            ...
                            <ext:Panel ID="IdsPanel" runat="server" Layout="FormLayout"
                                Padding="5" Title="ID's"  LabelWidth="200" >
                            <Items>
                            ...
                            <ext:Panel ID="ContactInformationPanel"  LabelWidth="200" runat="server" Title="Contact Information"
                                 Layout="FormLayout" Padding="5" >
                                <Items>
                                    <ext:CompositeField ID="cmfPrimaryHomeAddress" Height="25"  runat="server" 
                                        LabelWidth="500" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtPrimaryHomeAddress" Height="22" AllowBlank="false" DataIndex="PrimaryHomeAddress"
                                                runat="server" FieldLabel="Primary Home Address" Flex="1" ReadOnly="true">
                                                <Listeners>
                                                    <Focus Handler="#{winAddressDetailA1}.show();" />
                                                </Listeners>    
                                            </ext:TextField>
                                            <ext:Button ID="btnBrowsePrimAddress" Height="22" runat="server" Width="70" Text="...">
                                                <Listeners>
                                    ...
                                    <ext:CompositeField ID="cmfSecondaryHomeAddress" Height="25" runat="server" FieldLabel="Secondary Home Address"
                                        LabelWidth="500" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtSecondaryHomeAddress" Height="22" DataIndex="SecondaryHomeAddress" runat="server"
                                                FieldLabel="Secondary Home Address" AllowBlank="true" Flex="1" ReadOnly="false">
                                                <Listeners>
                                                    <Focus Handler="#{winAddressDetailA2}.show();" />
                                                </Listeners>    
                                            </ext:TextField>
                                            <ext:Button ID="btnBrowseSecAddress" Height="22" runat="server" Width="70" Text="...">
                                                <Listeners>
                                    ...
                                    <ext:CompositeField LabelPad="10"  ID="CellphoneNumber" Height="20" runat="server" FieldLabel="Cellphone Number"
                                        LabelWidth="500" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtCellTelCodes" ReadOnly="true" Height="20" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField4" runat="server" Height="20" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtCellAreaCode" MaxLength="5" MinLength="1" MaskRe="/\d/" AllowBlank="true" Height="20" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField5" runat="server" Height="20" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtCellPhoneNumber" Height="20" MaskRe="/\d/" MinLength="5" MaxLength="7"  AllowBlank="true" runat="server" Flex="1" />
                                        </Items>
                                    </ext:CompositeField>
                                    <ext:CompositeField LabelPad="10"  ID="TelephoneNumber" Height="20" runat="server" FieldLabel="Telephone Number"
                                        LabelWidth="500" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtTelCodes" Height="20" ReadOnly="true" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField6" Height="20" runat="server" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtTelAreaCode" MaxLength="4" MinLength="1" MaskRe="/\d/" Height="20" AllowBlank="true" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField7" Height="20" runat="server" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtTelPhoneNumber" Height="20" MaskRe="/\d/" MinLength="5" MaxLength="7" AllowBlank="true" runat="server" Flex="1" />
                                        </Items>
                                    </ext:CompositeField>
                            ...
                            <ext:Panel LabelWidth="200"  ID="EmploymentInformationPanel"
                                ...
                                <Items>
                                    <ext:CompositeField ID="cmfEmployer" Height="25" runat="server" FieldLabel="Employer" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtEmployerName" Height="22" runat="server" Flex="1" />
                                            <ext:Button ID="Button15" Text="Browse.." Height="22" runat="server" Width="60">
                                                <Listeners>
                                    ...
                                    <ext:TextField ID="txtEmploymentAddress" ReadOnly="true" FieldLabel="Employment Address"
                                        runat="server" AnchorHorizontal="60%" />
                                    <ext:CompositeField LabelPad="10" Height="20" ID="EmpTelephoneNumber" ReadOnly="true" runat="server"
                                        FieldLabel="Telephone Number" LabelWidth="500" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtEmpTelCode" Height="20" ReadOnly="true" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField8" Height="20" runat="server" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtEmpAreaCode" Height="20" ReadOnly="true" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField9" Height="20" runat="server" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtEmpPhoneNumber" Height="20" ReadOnly="true" runat="server" Flex="1" />
                                        </Items>
                                    </ext:CompositeField>
                                    <ext:CompositeField LabelPad="10" Height="20" ID="EmpFaxNumber" ReadOnly="true" runat="server"
                                        FieldLabel="Fax Number" LabelWidth="500" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtEmpFaxTelCode" Height="20" ReadOnly="true" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField10" Height="20" runat="server" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtEmpFaxAreaCode" Height="20" ReadOnly="true" runat="server" Width="75" />
                                            <ext:DisplayField ID="DisplayField11" Height="20" runat="server" Text="-" Cls="dot-label" />
                                            <ext:TextField ID="txtEmpFaxNumber" Height="20" ReadOnly="true" runat="server" Flex="1" />
                                        </Items>
                                    </ext:CompositeField>
                            ...
                            <ext:Panel ID="OtherSourceOfIncomePanel" AutoHeight="true" runat="server" 
                                Title="Other Sources of Income" Layout="FitLayout">
                                <Items>
                            ...
                            <ext:Panel ID="SpouseInformationPanel" LabelWidth="200" runat="server" Title="Spouse Information"
                                Layout="FormLayout" Padding="5" >
                                <Items>
                                    <ext:Hidden ID="SpouseID" runat="server" />
                                    <ext:CompositeField ID="CompositeField1" Height="25" runat="server" AnchorHorizontal="60%">
                                        <Items>
                                            <ext:TextField ID="txtSpouseName" Height="22" FieldLabel="Name" runat="server"
                                                Flex="1" ReadOnly="true">
                                                <Listeners>
                                                    <Focus Handler="#{wndSpouseNameDetail}.show();" />
                                                </Listeners>
                                            </ext:TextField>
                                            <ext:Button ID="Button1" Text="..." Height="22" runat="server" Width="60">
                                                <Listeners>
            ...
            <ext:FormPanel runat="server" LabelWidth="200" MonitorPoll="500" MonitorValid="true">
                <Items>
                    <ext:ComboBox ID="cmbSourceOfIncome" runat="server" FieldLabel="Source of Income" AnchorHorizontal="95%"
                        DisplayField="Name" ValueField="Id" StoreID="SourceOfIncomeStore" ForceSelection="true" Editable="false"
                        AllowBlank="false" />
                    <ext:TextField ID="txtAmount" AllowBlank="false" MaskRe="/[0-9\-\,\.]/" DataIndex="Amount" runat="server" FieldLabel="Amount"
                        AnchorHorizontal="95%">
                        <Listeners>
                            <Change Handler="var value = this.getValue().replace(/,/g,'');value = value.replace(/[]/g, '');this.setValue(Ext.util.Format.number(value, '0,0.00'));" />
                            <%--<Change Handler="this.setValue(Ext.util.Format.number(newValue.replace(/[]/g, ''), '0,000.00'));" />--%>
            ...
            <ext:FormPanel Padding="10" runat="server" ID="frmPersonNameDetail" LabelWidth="120"
                ...
                <Buttons>
                    <ext:Button runat="server" ID="btnSavePersonNameDetail" Text="Done" Icon="Accept">
                        <DirectEvents>
                            <Click OnEvent="btnSavePersonNameDetail_Click" Before="return #{frmPersonNameDetail}.getForm().isValid();" />
                        </DirectEvents>
                    </ext:Button>
    ...
    <ext:Window ID="wndSpouseNameDetail" Modal="true" runat="server" Collapsible="true" Height="300"
        Icon="Application" Title="Spouse Name" Width="350" Hidden="true">
        <Items>
            <ext:FormPanel Padding="10" runat="server" ID="frmSpouseNameDetail" LabelWidth="170"
                MonitorValid="true" MonitorPoll="500">
                <Items>
    ...
    <ext:Window ID="wndDistrictList" Modal="true" runat="server" Collapsible="true" AutoHeight="true"
             ...
             <ext:FormPanel Padding="10" runat="server" ID="frmAddressA1" LabelWidth="120" MonitorValid="true">
                <Items>
                    ...
                    <ext:ComboBox runat="server" ID="cmbCountryA1" FieldLabel="Country" AnchorHorizontal="100%"
                        ...
                        ForceSelection="true" TriggerAction="All" SelectOnFocus="true" AllowBlank="false"
                        StoreID="CountryStore" />
                    <ext:TextField ID="txtPostalCodeA1" runat="server" FieldLabel="Postal Code" MinLength="4" MaskRe="/\d/" MaxLength="4" AnchorHorizontal="100%" AllowBlank="false">
                    </ext:TextField>
                </Items>
                <BottomBar>
        ...
        <Items>
             <ext:FormPanel Padding="10" runat="server" ID="frmAddressA2" LabelWidth="120" MonitorValid="true">
                <Items>
                    ...
                    <ext:TextField ID="txtProvinceA2" LabelSeparator=":" runat="server" FieldLabel="Province" AnchorHorizontal="100%" AllowBlank="true">
                    </ext:TextField>
                    <ext:ComboBox runat="server" ID="cmbCountryA2" FieldLabel="Country" AnchorHorizontal="100%"
                        DisplayField="Name" ValueField="Id" Editable="false" TypeAhead="true" Mode="Local"
                        ForceSelection="true" TriggerAction="All" SelectOnFocus="true" AllowBlank="false"
                        StoreID="CountryStore" />
                    <ext:TextField ID="txtPostalCodeA2" runat="server" FieldLabel="Postal Code" MaxLength="4" MinLength="4" MaskRe="/\d/" AnchorHorizontal="100%" AllowBlank="false">
                    </ext:TextField>
                </Items>
                <BottomBar>
        ...
        <Items>
             <ext:FormPanel Padding="10" runat="server" ID="frmAddressB1" LabelWidth="120" MonitorValid="true">
                <Items>
                    ...
                    <ext:TextField ID="txtProvinceB1" LabelSeparator=":" runat="server" FieldLabel="Province" AnchorHorizontal="100%" AllowBlank="true">
                    </ext:TextField>
                    <ext:ComboBox runat="server" ID="cmbCountryB1" FieldLabel="Country" AnchorHorizontal="100%"
                        DisplayField="Name" ValueField="Id" Editable="false" TypeAhead="true" Mode="Local"
                        ForceSelection="true" TriggerAction="All" SelectOnFocus="true" AllowBlank="false"
                        StoreID="CountryStore" />
                    <ext:TextField ID="txtPostalCodeB1" runat="server" FieldLabel="Postal Code" MinLength="4" MaskRe="/\d/" MaxLength="4" AnchorHorizontal="100%" AllowBlank="false">
                    </ext:TextField>
                </Items>
                <BottomBar>

...
using BusinessLogic;
using Ext.Net;
using LendingApplication.Applications.LoanUseCases;

namespace LendingApplication.Applications.DemandLetterUseCases
{
    public partial class FirstDemandLetter : ActivityPageBase
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                var demandLetterId = int.Parse(Request.QueryString["id"]);
                RetreiveHeaderDetails();
                FillFields(demandLetterId);
            }
        ...
        private void FillFields(int demandLetterId)
        {
            var demandLetter = ObjectContext.DemandLetters.SingleOrDefault(entity => entity.Id == demandLetterId);
            var selectedLoanId = demandLetter.FinancialAccountId;
            ...
            //Last Payment Made
            var finAcctTran = ObjectContext.FinAcctTrans.Where(entity => entity.FinancialAccountId == selectedLoanId && entity.FinancialAcctTransTypeId == FinlAcctTransType.AccountPaymentType.Id).OrderByDescending(entity => entity.TransactionDate);
            if (finAcctTran.Count() > 0)
            {
                var lastFinAcctTrans = finAcctTran.FirstOrDefault();
                lblLastPaymentMade.Text = lastFinAcctTrans.TransactionDate.ToString("MMMM dd, yyyy");
            }
        }
        ...
        public class DueDatesModel 
        {
            ...
            public string _DueDate
            {
                get
                {
                    return this.DueDate.ToString("MMMM dd, yyyy");
                }
            }
